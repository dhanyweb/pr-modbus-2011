...
static unsigned int compute_response_length(modbus_param_t *mb_param,
	...
	case FC_READ_INPUT_REGISTERS:
		/* Header + 2 * nb values */
		length = 3 + 2 * (query[offset + 4] << 8 | query[offset + 5]);
		break;
...
static int build_query_basis_rtu(int slave, int function, int start_addr,
...
static int build_query_basis_tcp(int slave, int function, int start_addr,
		int nb, uint8_t *query) {
...
static int build_query_basis(modbus_param_t *mb_param, int slave, int function,
		int start_addr, int nb, uint8_t *query) {
	if (mb_param->type_com == RTU)
		return build_query_basis_rtu(slave, function, start_addr, nb, query);
	else
		return build_query_basis_tcp(slave, function, start_addr, nb, query);
}
...
static int modbus_send(modbus_param_t *mb_param, uint8_t *query,
	...
	if (mb_param->type_com == RTU) {
		/*
		 * this is the 2/3 extension of solving full/half-duplex problem
		 */
		sgpio_set2write();
...
static uint8_t compute_query_length_header(int function) {
	...
	if (function <= FC_FORCE_SINGLE_COIL || function
			== FC_PRESET_SINGLE_REGISTER)
		/* Read and single write */
		length = 4;
	else if (function == FC_FORCE_MULTIPLE_COILS || function
...
static int compute_query_length_data(modbus_param_t *mb_param, uint8_t *msg) {
	...
	if (function == FC_FORCE_MULTIPLE_COILS || function
			== FC_PRESET_MULTIPLE_REGISTERS)
 ...
 */
static int receive_msg(modbus_param_t *mb_param, int msg_length_computed,
...
/* Receives the response and checks values (and checksum in RTU).
 ...
 these values. */
static int modbus_receive(modbus_param_t *mb_param, uint8_t *query,
		uint8_t *response) {
	int ret;
	int response_length;
	...
	if (ret == 0) {
		...
		case FC_READ_INPUT_STATUS:
			...
			query_nb_value = (query_nb_value / 8) + ((query_nb_value % 8) ? 1
					: 0);
		...
		case FC_PRESET_MULTIPLE_REGISTERS:
			/* N Write functions */
			query_nb_value = (query[offset + 4] << 8) + query[offset + 5];
			response_nb_value = (response[offset + 4] << 8) | response[offset
					+ 5];
			break;
...
void modbus_manage_query(modbus_param_t *mb_param, const uint8_t *query,
	...
	case FC_READ_COIL_STATUS: {
		...
		if ((address + nb) > mb_mapping->nb_coil_status) {
			...
			resp_length = response_io_status(address, nb,
					mb_mapping->tab_coil_status, response, resp_length);
		}
	}
		break;
	case FC_READ_INPUT_STATUS: {
		...
		if ((address + nb) > mb_mapping->nb_input_status) {
			...
			response[resp_length++] = (nb / 8) + ((nb % 8) ? 1 : 0);
			resp_length = response_io_status(address, nb,
					mb_mapping->tab_input_status, response, resp_length);
		}
	}
		break;
	case FC_READ_HOLDING_REGISTERS: {
		...
		if ((address + nb) > mb_mapping->nb_holding_registers) {
			...
			for (i = address; i < address + nb; i++) {
				response[resp_length++] = mb_mapping->tab_holding_registers[i]
						>> 8;
				response[resp_length++] = mb_mapping->tab_holding_registers[i]
						& 0xFF;
			}
	...
	case FC_READ_INPUT_REGISTERS: {
		...
		if ((address + nb) > mb_mapping->nb_input_registers) {
			...
			response[resp_length++] = nb << 1;
			for (i = address; i < address + nb; i++) {
				response[resp_length++] = mb_mapping->tab_input_registers[i]
						>> 8;
				response[resp_length++] = mb_mapping->tab_input_registers[i]
						& 0xFF;
			}
	...
	case FC_FORCE_SINGLE_COIL:
		if (address >= mb_mapping->nb_coil_status) {
			...
			if (data == 0xFF00 || data == 0x0) {
				...
				printf(
						"Illegal data value %0X in force_single_coil request at address %0X\n",
						data, address);
				resp_length = response_exception(mb_param, &sft,
	...
	case FC_FORCE_MULTIPLE_COILS: {
		...
		if ((address + nb) > mb_mapping->nb_coil_status) {
			...
			set_bits_from_bytes(mb_mapping->tab_coil_status, address, nb,
					&query[offset + 7]);
	...
	case FC_PRESET_MULTIPLE_REGISTERS: {
		...
		if ((address + nb) > mb_mapping->nb_holding_registers) {
			...
			for (i = address, j = 0; i < address + nb; i++, j += 2) {
				...
				mb_mapping->tab_holding_registers[i] = (query[offset + 7 + j]
						<< 8) + query[offset + 8 + j];
			}
...
static int read_io_status(modbus_param_t *mb_param, int slave, int function,
	...
	query_length = build_query_basis(mb_param, slave, function, start_addr, nb,
			query);
...
int read_coil_status(modbus_param_t *mb_param, int slave, int start_addr,
	...
	status = read_io_status(mb_param, slave, FC_READ_COIL_STATUS, start_addr,
			nb, data_dest);
...
int read_input_status(modbus_param_t *mb_param, int slave, int start_addr,
	...
	status = read_io_status(mb_param, slave, FC_READ_INPUT_STATUS, start_addr,
			nb, data_dest);
...
static int set_single(modbus_param_t *mb_param, int slave, int function,
	...
	query_length = build_query_basis(mb_param, slave, function, addr, value,
			query);
...
int force_single_coil(modbus_param_t *mb_param, int slave, int coil_addr,
	...
	status
			= set_single(mb_param, slave, FC_FORCE_SINGLE_COIL, coil_addr,
					state);
...
int preset_single_register(modbus_param_t *mb_param, int slave, int reg_addr,
	...
	status = set_single(mb_param, slave, FC_PRESET_SINGLE_REGISTER, reg_addr,
			value);
...
int preset_multiple_registers(modbus_param_t *mb_param, int slave,
		int start_addr, int nb, const uint16_t *data_src) {
	int ret;
	int i;
	int query_length;
	int byte_count;
	...
	query_length = build_query_basis(mb_param, slave,
			FC_PRESET_MULTIPLE_REGISTERS, start_addr, nb, query);
	byte_count = nb * 2;
	query[query_length++] = byte_count;
...
int report_slave_id(modbus_param_t *mb_param, int slave, uint8_t *data_dest) {
	...
	query_length = build_query_basis(mb_param, slave, FC_REPORT_SLAVE_ID, 0, 0,
			query);
...
/* Initializes the modbus_param_t structure for RTU
 ...
 - stop_bits: 1, 2
 */
void modbus_init_rtu(modbus_param_t *mb_param, const char *device, int baud,
		const char *parity, int data_bit, int stop_bit) {
...
void modbus_set_error_handling(modbus_param_t *mb_param,
	...
	if (error_handling == FLUSH_OR_RECONNECT_ON_ERROR || error_handling
			== NOP_ON_ERROR) {
...
static int modbus_connect_rtu(modbus_param_t *mb_param) {
	...
	mb_param->fd = open(mb_param->device, O_RDWR | O_NOCTTY /*| O_NDELAY*/ | O_SYNC);
	if (mb_param->fd < 0) {
		perror("open");
...
static int modbus_connect_tcp(modbus_param_t *mb_param) {
	...
	ret = setsockopt(mb_param->fd, IPPROTO_TCP, IP_TOS, (const void *) &option,
			sizeof(int));
	if (ret < 0) {
		perror("setsockopt");
		close(mb_param->fd);
 ...
 */
int modbus_mapping_new(modbus_mapping_t *mb_mapping, int nb_coil_status,
		int nb_input_status, int nb_holding_registers, int nb_input_registers) {
	/* 0X */
	mb_mapping->nb_coil_status = nb_coil_status;
	mb_mapping->tab_coil_status = (uint8_t *) malloc(nb_coil_status
			* sizeof(uint8_t));
	...
	mb_mapping->tab_input_status = (uint8_t *) malloc(nb_input_status
			* sizeof(uint8_t));
	...
	memset(mb_mapping->tab_holding_registers, 0, nb_holding_registers
			* sizeof(uint16_t));
	...
	mb_mapping->tab_input_registers = (uint16_t *) malloc(nb_input_registers
			* sizeof(uint16_t));
